...
//    self.selectionView = [[PMSelectionView alloc] initWithFrame:CGRectInset(self.bounds, -innerPadding.width, -innerPadding.height)];
    self.selectionView = [[PMSelectionView alloc] initWithFrame:CGRectInset(CGRectMake(self.bounds.origin.x + monthPickerWidth, self.bounds.origin.y, self.bounds.size.width - monthPickerWidth, self.bounds.size.height), -innerPadding.width, -innerPadding.height)];
    ...
    //Month picker
    pickerArray = [NSArray arrayWithObjects:@"JAN", @"FEB", @"MARCH", @"APRIL", @"MAY", @"JUNE", @"JULY", @"AUG", @"SEP", @"OCT", @"NOV", @"DEC", nil];
    
    self.monthPicker = [[UIPickerView alloc] init];
    self.monthPicker.delegate = self;
    self.monthPicker.dataSource = self;
    self.monthPicker.frame = CGRectMake(0, (self.frame.size.height - 216) / 2, monthPickerWidth, 216*2);
    self.monthPicker.showsSelectionIndicator = NO;
...
//                                           , hDiff
//                                           , 30);
        CGRect dayHeaderFrame = CGRectMake(monthPickerWidth + floor(i * hDiff) - 1 + 10
                                           , headerHeight - shadow2Offset.height + 3
                                           , hDiff
                                           , 30);
...
//    NSInteger newFontSize = self.frame.size.width / 20;
    NSInteger newFontSize = self.frame.size.width / 40;
    ...
    if (!tView)
    {
        ...
        if ([[UIDevice currentDevice].model containsString:@"iPad"]) {
            if (row == [pickerView selectedRowInComponent:0])
                [tView setFont:[UIFont fontWithName:@"PorscheDesignFont" size:28]];
            else
                [tView setFont:[UIFont fontWithName:@"PorscheDesignFont" size:28]];
        }
        else {
            if (row == [pickerView selectedRowInComponent:0])
                [tView setFont:[UIFont fontWithName:@"PorscheDesignFont" size:18]];
            else
                [tView setFont:[UIFont fontWithName:@"PorscheDesignFont" size:18]];
        }
...
//    [components setTimeZone:[[NSCalendar currentCalendar] timeZone]];
//    [components setHour:1];
//    [self setCurrentDate:[[NSCalendar currentCalendar] dateFromComponents:components]];
    [self setCurrentDate:[self.currentDate dateByAddingMonths:row+1 - currentMonth]];
}
...
//    NSInteger daysSinceMonthStart = [date timeIntervalSinceDate:monthStartDate] / (60 * 60 *24);
    float f = [date timeIntervalSinceDate:monthStartDate] / (60 * 60 *24);
    NSInteger daysSinceMonthStart = lroundf(f);
...
//    CGFloat xInCalendar = point.x - 2;
    CGFloat xInCalendar = point.x - 2 - monthPickerWidth;
...
//    if (point.y > headerHeight + vDiff) // select date in calendar
    if (point.y > headerHeight + vDiff / 4) // select date in calendar
    {
...
//    if (point.y > headerHeight + vDiff) // select date in calendar
    if (point.y > headerHeight + vDiff / 4 && point.x > monthPickerWidth) // select date in calendar
    {
        [self periodSelectionStarted:point];
        return;
    }
...
//                                            , hDiff, self.font.pointSize);
        CGRect dayHeader2Frame = CGRectMake(monthPickerWidth + i * hDiff + 7
                                            , headerHeight + vDiff / 4 - shadow2Offset.height + 5
                                            , hDiff, self.font.pointSize);
//        UIColor *color = [UIColor colorWithWhite:0.6f alpha:1.0f];
        UIColor *color = [UIColor colorWithWhite:0.2f alpha:1.0f];
...
//                                                    , hDiff, self.font.pointSize);
                CGRect dayHeader2Frame = CGRectMake(monthPickerWidth + floor(j * hDiff) - 1 + 7
                                                    , headerHeight + i * vDiff + 0.25*vDiff - shadow2Offset.height + 5
                                                    , hDiff, self.font.pointSize);
                UIColor *color = nil;
...
//                    color = [UIColor colorWithRed: 0.98 green: 0.24 blue: 0.09 alpha: 1];
                    color = [UIColor colorWithRed: 3.0/255 green: 127.0/255 blue: 216.0/255 alpha: 1];
                }
                else 
                {
...
//                                                , hDiff, self.font.pointSize);
            CGRect dayHeader2Frame = CGRectMake(monthPickerWidth + floor(i * hDiff) - 1 + 7
                                                , headerHeight + finalRow * vDiff + 0.25*vDiff - shadow2Offset.height + 5
                                                , hDiff, self.font.pointSize);
//            UIColor *color = [UIColor colorWithWhite:0.6f alpha:1.0f];
            UIColor *color = [UIColor colorWithWhite:0.2f alpha:1.0f];
            drawString( string, dayHeader2Frame, color );
    ...
    if (finalRow < 6) {
        for ( int i = finalRow + 1; i < 6; i++) {
            for ( int j = 0; j < 7; j++) {
                int day = (7 - weekdayOfNextFirst) + (i - finalRow - 1) * 7 + j + 1;
                if (weekdayOfNextFirst == 0) {
                    day = (i - finalRow - 1) * 7 + j + 1;
                }
                NSString *string = [NSString stringWithFormat:@"%d", day];
                CGRect dayHeader2Frame = CGRectMake(monthPickerWidth + floor(j * hDiff) - 1 + 7
                                                    , headerHeight + i * vDiff + 0.25*vDiff - shadow2Offset.height + 5
